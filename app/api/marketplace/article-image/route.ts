...
import { verifyPublishRequestWithDigest } from "@/lib/publish-auth";
import {
...
import { articleImageUploadDigestFromFile } from "@/lib/publish-payload";
...
export async function POST(request: Request) {
  ...
  const publishAuth = await verifyPublishRequestWithDigest(
    request,
    articleImageUploadDigestFromFile(fileValue),
  );
  if (!publishAuth) {
    return NextResponse.json(
      { error: "Connect your wallet and sign to upload this image" },
      { status: 401 },
    );
  }
